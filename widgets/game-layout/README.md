...
### State variants
- **Карты руки:** обычные, при наведении/клике увеличивают контраст и показывают тултип. Непроигрываемые отображаются с приглушённой насыщенностью. Под художественным текстом теперь выводится шанс успеха и раздельные описания исходов при удаче и провале.
...
- **Интерактивный виджет:** `data-stage` переключает между `story`, `player` и `event`. Вступление выводит сценарные фразы, иллюстрацию справа и кнопку «Далее», ход игрока показывает руку/счётчики и кнопку «Закончить ход», событие — выбор/результат и кнопку «Продолжить».
...
- `scenario.intro` — заголовок и массив фраз, отображаемых в блоке вступления. Опциональное поле `flavor_img` даёт ссылку на иллюстрацию, которая показывается в интерактивном виджете рядом с текстом.
...
- `scenario.firstTask.flavor_img` — изображение, показываемое рядом с карточкой первого задания в панели story.
